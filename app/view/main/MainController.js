/**
 * This class is the main view for the application. It is specified in app.js as the
 * "autoCreateViewport" property. That setting automatically applies the "viewport"
 * plugin to promote that instance of this class to the body element.
 *
 * TODO - Replace this content of this view to suite the needs of your application.
 */
Ext.define('wealthmgr.view.main.MainController', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.main',
	
	init: function() {
		this.control({ // aqui definimos los objetos y eventos que vamos a controlar
        		
            	'app-main': {//evento del formulario main
            			afterrender: function(c) {
									var main = wealthmgr.app.getController('generalController');
									
									Ext.getCmp('mainLogon').setText(wealthmgr.app.datosLogin.email); // pone el nombre de usuario abajo a la derecha
									main.fireEvent('cargarPermisos'); // lo captura mainmenu controller

						}
				}
		});
    	
	}
});
